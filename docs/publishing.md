...
Go to **Actions > CI > Run workflow**, select the version bump type, and click **Run workflow**. The pipeline will run the release script and push the tag, which triggers the publish job.
...
| `release-dispatch` | `workflow_dispatch` | Build, test, run release script |
| `publish` | tag `v*` | Publish to Marketplace, create GitHub Release with `.vsix` |

Release notes in the GitHub Release come from `RELEASE_NOTES.md` (generated by the release script).
